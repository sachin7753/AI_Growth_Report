...
def get_ai_recommendations(status, age_m, wfh_p, hfa_p, bmi):
    ...
    if status in ["Obese","Overweight"]:
        recs += ["- Balanced meals with vegetables, fruits, lean proteins.",
        ...
        recs += ["- Continue balanced diet & regular meals.",
...
def generate_report(age_m, ht, wt, sex, model, scaler):
    ...
    bmi = wt/((ht/100)**2)
    who_msgs=[]
    if wfh_p<3: who_msgs.append((f"Wasting risk (P{wfh_p:.1f})", colors.red))
    elif wfh_p>85: who_msgs.append((f"Overweight risk (P{wfh_p:.1f})", colors.red))
    else: who_msgs.append(("Wt-for-height healthy.", colors.green))
    if hfa_p<3: who_msgs.append((f"Stunting risk (P{hfa_p:.1f})", colors.red))
    else: who_msgs.append(("Ht-for-age healthy.", colors.green))
...
def create_pdf_report(child_name, age_months, report):
    ...
    # PLOT CHARTS in-memory
    hfa_buf = BytesIO(); wfh_buf = BytesIO()
    ...
    plt.xlabel("Percentile"); plt.ylabel("Height (cm)"); plt.title("Height-for-Age Percentile"); plt.legend(); plt.tight_layout(); plt.savefig(hfa_buf, format='PNG'); plt.close()
    hfa_buf.seek(0)
    plt.figure(figsize=(6,4))
    ...
    plt.xlabel("Percentile"); plt.ylabel("Weight (kg)"); plt.title("Weight-for-Height Percentile"); plt.legend(); plt.tight_layout(); plt.savefig(wfh_buf, format='PNG'); plt.close()
    wfh_buf.seek(0)
    # Embed charts
    c.showPage()
...
with st.sidebar:
    ...
    age_months = st.number_input("Age in Months", min_value=0, max_value=60, value=24)
...
if generate_button and growth_model and scaler:
    ...
    if report:
        ...
        with col_who:
            ...
            for msg,color in report['who_msgs']: st.markdown(f"- {msg}")
        with col_ai:
        ...
        if parent_email: send_email_report(parent_email, pdf_buffer, child_name)
